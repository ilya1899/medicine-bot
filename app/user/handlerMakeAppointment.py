...
async def callback_makeAppointment(callback: CallbackQuery):
    ...
    if city:
        await callback.message.edit_text(f'Вы ищете специалиста в своем городе? ({city})', reply_markup=kbInline.makeAppointment)
    else:
...
async def callback_noMyCity(callback: CallbackQuery):
    ...
    await callback.message.edit_text('Выберите страну',
                                     reply_markup=await kbInline.getKeyboardCountryOrCity(countries, 'makeAppointmentCountry'))
...
async def callback_makeAppointmentCountry(callback: CallbackQuery):
    ...
    if cities:
        await callback.message.edit_text('Выберите город',
                                         reply_markup=await kbInline.getKeyboardCountryOrCity(callback, 'makeAppointmentCity'))
    else:
